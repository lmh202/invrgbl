...
class WaymoProcessor(object):
    ...
    def save_lidar_(self, frame, file_idx, frame_idx):
    ...
    def save_lidar(self, frame, file_idx, frame_idx):
        ...
        (
            ...
            elongation,
            laser_ids,
        ) = convert_range_image_to_point_cloud_flow( #convert_range_image_to_point_cloud( #
            frame,
            range_images,
            range_images_flow,
            camera_projections,
        ...
        points_1 = np.concatenate(points_1, axis=0)
        #ground_label_1 = get_ground_np(points_1)
        intensity_1 = np.concatenate(intensity_1, axis=0)
        elongation_1 = np.concatenate(elongation_1, axis=0)
        laser_ids_1 = np.concatenate(laser_ids_1, axis=0)
        flows_1 = np.concatenate(flows_1, axis=0)

        origins = np.concatenate([origins,origins_1], axis=0)
        points = np.concatenate([points,points_1], axis=0)
        ground_label = get_ground_np(points)
        intensity = np.concatenate([intensity,intensity_1], axis=0)
        elongation = np.concatenate([elongation,elongation_1], axis=0)
        laser_ids = np.concatenate([laser_ids,laser_ids_1], axis=0)
        flows = np.concatenate([flows,flows_1], axis=0)
