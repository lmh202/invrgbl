...
if __name__ == "__main__":
    ...
    if args.scene_ids is not None:
        ...
        # parse the split file (supports optional header and blank/comment lines)
        with open(args.split_file, "r") as f:
            split_lines = f.readlines()

        if split_lines and "scene_id" in split_lines[0].lower():
        ...
        for raw_line in split_lines:
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid scene id '{token}' in split file {args.split_file}. "
                    "Each line must start with an integer scene index."
                ) from exc
    else:
